...
class PluginSettings {
    ...
    private wpdb $db;
    private ?bool $had_amazon_api_key = null;

    public function __construct( wpdb $db = null ) { // phpcs:ignore
        global $wpdb;
    ...
    public function had_amazon_api_key(): bool {
        if ( $this->had_amazon_api_key !== null ) {
            return $this->had_amazon_api_key;
        }

        $raw_settings = get_option( HOSTINGER_AFFILIATE_PLUGIN_SLUG, array() );

        $this->had_amazon_api_key = is_array( $raw_settings ) && ! empty( $raw_settings['amazon']['api_key'] );
