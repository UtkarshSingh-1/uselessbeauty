...
class Events {
    ...
    public function affiliate_created( string $layout = '', string $marketplace = 'amazon' ) {
        ...
        $transient_key = "amplitude_create_{$marketplace}_{$layout}";
        ...
        set_transient( $transient_key, true, self::TEN_MINUTES_IN_SECONDS );
    }
    ...
    private function send_request( string $endpoint, array $params = array(), string $marketplace = 'amazon' ): array {
        ...
        if ( $marketplace === 'amazon' ) {
            $combined_params = array_merge( $params, array( 'scraping' => true ) );
        }
