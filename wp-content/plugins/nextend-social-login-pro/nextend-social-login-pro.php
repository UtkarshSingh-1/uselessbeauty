...
class NextendSocialLoginPRO {
    ...
    public static $nslMinVersion = '3.1.21';
        ...
        if (!defined('NSL_PATH')) {
            ...
            if ($lastVersion != self::$version && version_compare(NextendSocialLogin::$version, self::$nslMinVersion, '>=')) {
                ...
                if (!empty($lastVersion)) {
                    ...
                    /**
                     * In 3.1.20 we updated our VK integration to use their new App type and endpoints, as new Apps can no longer be created on the old panel.
                     * The new Apps and the old Apps are not compatible, and they work only with the endpoints they were originally designed for.
                     * As the old apps are no longer compatible with the new flow, we need to inform the users with existing ( pre 3.1.20 ) VK configurations about that, they have to create a new App.
                     */
                    if (version_compare($lastVersion, '3.1.19', '<=') && isset(NextendSocialLogin::$providers['vk']) && isset(NextendSocialLogin::$providers['vk']->settings) && NextendSocialLogin::$providers['vk']->settings->get('tested')) {
                        NextendSocialLogin::$providers['vk']->settings->update(array(
                            'tested'                         => 0,
                            'show_new_app_requirement_error' => 1
                        ));
                    }
                }
