...
class Menus {
    ...
    public function render_plugin_content(): void {
        ...
        $initial_settings = array_merge(
            $this->plugin_settings->get_plugin_settings()->to_array(),
            array( 'had_amazon_api_key' => $this->plugin_settings->had_amazon_api_key() )
        );

        ?>
        <div id="affiliate-plugin-app" class="affiliate-plugin-app">
            <App
                    :initial-settings="<?php echo htmlspecialchars( json_encode( $initial_settings ) ); ?>">
            </App>
        </div>
        <?php
    }
