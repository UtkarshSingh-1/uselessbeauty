...
class ProductTableShortcode extends Shortcode {
    ...
    public function render_table_row( FeatureRow $feature_row, AsinRow $asin_row ): string {
        ...
        switch ( $feature_row->get_selected_value() ) {
            case 'title':
                $value = '<a href="' . $product_url . '" target="_blank" rel="sponsored noopener noreferrer">' . $this->render_product_title( $product ) . '</a>';
                break;
            case 'thumbnail':
                $value = '<a href="' . $product_url . '" target="_blank" rel="sponsored noopener noreferrer"><img src="' . $product->get_image_url() . '" alt="' . esc_attr( $product->get_title() ) . '"></a>';
                break;
            case 'price':
