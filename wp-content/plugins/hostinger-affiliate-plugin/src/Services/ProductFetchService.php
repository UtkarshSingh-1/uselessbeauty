...
class ProductFetchService {
    ...
    public function pull_bestsellers( string $keywords, string $layout ): mixed {
        ...
        if ( empty( $list ) ) {
            ...
            $asins = array();
            $items = array_slice( $response, 0, 3 );
    ...
    public function get_items( array $item_ids, string $marketplace = 'amazon' ): array|WP_Error {
        ...
        switch ( $marketplace ) {
            ...
            case 'amazon':
            default:
                $response = $this->amazon_api_factory->get_api_factory()->product_api()->product_data( $product_data_request );
                break;
        }
    ...
    public function search_items( string $keywords, string $marketplace = 'amazon' ): array|WP_Error {
        ...
        switch ( $marketplace ) {
            ...
            case 'amazon':
            default:
                $response = $this->amazon_api_factory->get_api_factory()->search_api()->search( $search_request );
                break;
        }
    ...
    private function parse_items( array $items, string $marketplace = 'amazon' ): array {
        $products            = array();
        $product_transformer = $this->product_transformer_factory->get_transformer( $marketplace );
