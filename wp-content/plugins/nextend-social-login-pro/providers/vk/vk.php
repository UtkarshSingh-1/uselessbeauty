...
class NextendSocialPROProviderVK extends NextendSocialProviderOAuth {
    ...
    public function __construct() {
        ...
        parent::__construct(array(
            'application_id'                 => '',
            'secure_key'                     => '',
            'show_new_app_requirement_error' => 0,
            ...
            switch ($key) {
                case 'tested':
                    if ($postedData[$key] == '1' && (!isset($newData['tested']) || $newData['tested'] != '0')) {
                        ...
                        if ($this->settings->get('show_new_app_requirement_error')) {
                            //if the test was successful, then we can assume that the user replaced the app, so we no longer need to display the error
                            $newData['show_new_app_requirement_error'] = 0;
                        }
                    } else {
                        $newData['tested'] = 0;
                    }
                ...
                case 'secure_key':
                    ...
                    if ($this->settings->get($key) !== $newData[$key]) {
                        ...
                        if ($this->settings->get('show_new_app_requirement_error')) {
                            //if the credentials changed, then we can assume that the user replaced the app, so we no longer need to display the error
                            $newData['show_new_app_requirement_error'] = 0;
                        }
                    }
                ...
                case 'load_style':
                    $newData[$key] = trim(sanitize_text_field($value));
                    break;
                case 'show_new_app_requirement_error':
    ...
    public function getAuthUserData($key) {
        switch ($key) {
            ...
            case 'picture':
                if (!in_array($this->authUserData['avatar'], array(
                    ...
                    return $this->authUserData['avatar'];
                } else {
                    return '';
                }
        }
    ...
    private function maybeDisplayAppError() {
        $show_new_app_requirement_error = $this->settings->get('show_new_app_requirement_error');
        if ($show_new_app_requirement_error) {
            if (!isset($_GET['page']) || $_GET['page'] != 'nextend-social-login' || !isset($_GET['view']) || $_GET['view'] != 'provider-vk') {
                add_action('admin_notices', function () {
                    echo '<div class="error">
                        <p>' . sprintf(__('%1$s: %2$s introduced new App types and modified their endpoints which are not compatible with the old Apps.%3$sWe detected that, you are currently using an old %2$s App, so we had to disable your %2$s configuration. To resolve the problem, please create a new App based on our %4$s guide, and replace the credentials on the %5$s tab with your new App credentials.', 'nextend-facebook-connect'), '<b>Nextend Social Login</b>', __('VKontakte', 'nextend-facebook-connect'), '<br>', __('Getting Started', 'nextend-facebook-connect'), __('Settings', 'nextend-facebook-connect')) . '</p>
                        <p class="submit"><a href="' . NextendSocialLoginAdmin::getAdminUrl('provider-vk') . '" class="button button-primary">' . sprintf(__('Open %1$s - %2$s', 'nextend-facebook-connect'), 'VKontakte', __('Getting Started', 'nextend-facebook-connect')) . '</a></p>
                    </div>';
                });
            }
        }
